...
export function compileAppleMobileConfig(input={}){
  ...
  if(Array.isArray(policy.dnsServers)&&policy.dnsServers.length){
    const protocol=(policy.dnsProtocol||"HTTPS").toUpperCase();
    const dns={
      DNSProtocol:protocol,
      ServerAddresses:policy.dnsServers
    };
    if(protocol==="HTTPS"){
      if(policy.dnsServerUrl) dns.ServerURL=policy.dnsServerUrl;
      else warnings.push({code:"APPLE_DNS_SERVER_URL_REQUIRED",message:"DNS over HTTPS ต้องมี Server URL ที่ถูกต้อง"});
    }
